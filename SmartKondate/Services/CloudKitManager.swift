...
final class CloudKitManager {
    ...
    /// SwiftData ゾーン内に新しい CKShare を生成して保存する
    func prepareShare() async throws -> CKShare {
        let privateDB = container.privateCloudDatabase
        let zoneID = CKRecordZone.ID(zoneName: "com.apple.coredata.cloudkit.zone", ownerName: CKCurrentUserDefaultName)
        
        // 毎回一意の ID でルートレコードと Share を新規作成
        let uniqueID = UUID().uuidString
        let rootRecordID = CKRecord.ID(recordName: "KondatePatternRoot_\(uniqueID)", zoneID: zoneID)
        let shareID = CKRecord.ID(recordName: "Share_\(uniqueID)", zoneID: zoneID)
        
        let rootRecord = CKRecord(recordType: "CD_KondatePattern", recordID: rootRecordID)
